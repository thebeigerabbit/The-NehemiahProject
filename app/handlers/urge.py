"""
/urge handler + follow-up callback handler.
"""
from telegram import Update, InlineKeyboardButton, InlineKeyboardMarkup
from telegram.ext import ContextTypes, CallbackQueryHandler
from app.database import get_db
from app.services.user_service import get_user_by_telegram_id, get_accepted_partners
from app.services.urge_service import (
    validate_urge_reason, count_recent_urges, create_urge,
    resolve_urge, parse_urge_command, get_urge,
)
from app.services.checkin_service import (
    get_pending_checkin, create_checkin_record, process_yes_response, process_no_response,
)
from app.services.notification_service import (
    notify_partners_urge, notify_partners_urge_spam, notify_partners_failure,
)
...
async def urge_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    telegram_id = str(update.effective_user.id)
    raw_text = update.message.text or ""

    reason = parse_urge_command(raw_text)

    if not reason:
        await reply(update,
            " Invalid Format\n\n"
            "Usage: /urge reason: your reason here\n\n"
            "Example:\n"
    ...
    with get_db() as db:
        ...
        urge = create_urge(db, user, reason)
...
async def urge_followup_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle inline button response to urge follow-up."""
    query = update.callback_query
    await query.answer()

    data = query.data  # format: urge_followup:(urge_id):(resolution)
    parts = data.split(":")
    if len(parts) != 3 or parts[0] != "urge_followup":
        return
    ...
    with get_db() as db:
        ...
        if resolution == "fallen":
            ...
            # Re-trigger urge flow (notify partners again, schedule another follow-up)
            from app.services.urge_service import create_urge as re_urge
            new_urge = re_urge(db, user, f"[Continued] {urge.reason}")
